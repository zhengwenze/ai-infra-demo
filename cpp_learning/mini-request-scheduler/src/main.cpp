...
// 实验参数
constexpr int kTotalRequests = 120;  // 总请求数（约 1 秒完成，方便本地实验）
constexpr int kProducerThreads = 2;  // 生产者线程数（推送请求）
constexpr int kConsumerThreads = 2;  // 消费者线程数（执行推理）
constexpr int kBatchSize = 4;        // 批大小
constexpr int kQueueCapacity = 16;   // 小队列更容易观察排队和背压
constexpr int kProgressInterval = 20;
...
// ============================================================
// FakeModel：模拟 LLM 推理
// 按 max_tokens × 0.05ms 睡眠
// ============================================================

class FakeModel {
public:
  std::string generate(const Request &req) {
    int sleep_ms = static_cast<int>(req.max_tokens * 0.05);
...
void producerThread(BlockingQueue &queue, int count) {
  ...
  const std::vector<std::string> prompts = {
      "What is KV Cache?",       "What is Continuous Batching?",
      "What is PagedAttention?", "Explain vLLM architecture",
      "What is TensorRT-LLM?",   "How does Flash Attention work?",
      "Compare GPT and BERT",    "What is speculative decoding?",
  };
  ...
  while (true) {
    ...
    for (Request &req : batch) {
      ...
      // 汇总到全局结果表
      {
        ...
        const std::size_t completed = g_completed.size();
        if (completed % kProgressInterval == 0 ||
            completed == static_cast<std::size_t>(kTotalRequests)) {
          std::cout << "[Progress] " << completed << "/" << kTotalRequests
                    << " requests completed\n";
        }
      }
    }
...
void printReport() {
  ...
  auto print_row = [&](const char *name, const std::vector<double> &sorted) {
    printf("  %-18s  min=%7.2f  P50=%7.2f  P90=%7.2f  "
           "P95=%7.2f  P99=%7.2f  max=%7.2f  avg=%7.2f\n",
           name, sorted.front(), stats::percentile(sorted, 0.50),
           stats::percentile(sorted, 0.90), stats::percentile(sorted, 0.95),
           stats::percentile(sorted, 0.99), sorted.back(),
...
int main() {
  ...
  std::cout
      << "=============================================================\n";
  std::cout << " Mini Request Scheduler : " << kProducerThreads
            << " producers + " << kConsumerThreads << " consumers\n";
  std::cout
      << "=============================================================\n\n";
  ...
  if (static_cast<int>(g_completed.size()) != kTotalRequests) {
    std::cerr << "ERROR: lost " << (kTotalRequests - (int)g_completed.size())
              << " requests!\n";
    return 1;
  }
  return 0;
